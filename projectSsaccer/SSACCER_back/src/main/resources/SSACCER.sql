...
CREATE TABLE MATCH_BOARD ( -- 경기 수락 시에, 팀 별로 생성
	board_id INT AUTO_INCREMENT, -- 보드 고유 번호
	my_team_id INT NOT NULL, -- 우리팀 고유번호
    opposing_team_id INT NOT NULL, -- 상대팀 고유번호
	...
	result ENUM('win', 'loss', 'draw') DEFAULT 'draw',
	create_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(board_id),
    FOREIGN KEY(my_team_id) REFERENCES TEAM(team_id),
    FOREIGN KEY(opposing_team_id) REFERENCES TEAM(team_id)
);

CREATE TABLE MATCH_BOARD_DETAIL ( -- 경기 상세
    ...
    FOREIGN KEY (board_id) REFERENCES MATCH_BOARD (board_id),
    FOREIGN KEY (user_id) REFERENCES USER (user_id)
);

CREATE TABLE MATCH_BOARD_REVIEW ( -- 경기 피드백
