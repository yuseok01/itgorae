...
//	}
	...
	public ResponseEntity<?> list(@Parameter(description = "검색 조건") @ModelAttribute reviews condition){
		List<videoBoard> list = boardService.search(condition); //3.검색 조회
		
		if(list == null || list.size() == 0) {
			return new ResponseEntity<Void>(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<videoBoard>>(list, HttpStatus.OK);
	}//4. 리스트를 실어 보낼꺼고 잘 보냈는지 ok 확인
	//5. 자바 객체 알아서 제이슨으로 바꿔줌 
	
	//1.게시글 상세 보기 get방식 id 들고가자 
	@GetMapping("/board/{id}")
	public ResponseEntity<videoBoard> detail(@PathVariable("id") int id){
		//2.parhvariable id안적어도 되지만 적어주자 
		videoBoard board = boardService.readBoard(id); //3.조회수도 하나 증가 하더라!
		//4.가져왔는데 board 가 null이면 예외처리를 해줘라! 404 처리! (직접 해볼것 ㅎ)
		return new ResponseEntity<videoBoard>(board, HttpStatus.OK);
	}
	
	//1. 게시글 등록 (FORM 형식으로 넘어왔을때 ,JSON형태로 오는건 수정할때 할꺼임 둘다이해하자)
	@PostMapping("/board")
	public ResponseEntity<?> write(@ModelAttribute videoBoard board){
		...
		return new ResponseEntity<videoBoard>(board, HttpStatus.CREATED);
	}
	
	//1.게시글 수정 json형태로 해보기   application/json -> api-board/board/4 4번글 수정 
//	@PutMapping("/board")
	@PutMapping("/board/{id}")
	public ResponseEntity<Void> update(@PathVariable("id") int id, @RequestBody videoBoard board){
